# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""

# bitcoin-cli -regtest decoderawtransaction $transaction

# echo -n 522102da2f10746e9778dd57bd0276a4f84101c4e0a711f9cfd9f09cde55acbdd2d1912102bfde48be4aa8f4bf76c570e98a8d287f9be5638412ab38dede8e78df82f33fa352ae \
# | xxd -r -p \
# | openssl dgst -sha256 -binary \
# | openssl dgst -rmd160

# build the p2sh address from the redeem script
# a5d106eb8ee51b23cf60d8bd98bc285695f233f3
# OP_HASH160 <hash> OP_EQUAL

# In hex:
# a9 = OP_HASH160
# 14 = next is 20 bytes
# 87 = OP_EQUAL
# a914a5d106eb8ee51b23cf60d8bd98bc285695f233f387

printf "%s" "2NByn92W1vH5oQC1daY69F5sU7PEStKKQBR"